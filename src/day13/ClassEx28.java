...
class Line extends ShapeAbst{
	...
	@Override
	public void draw() {		//	[next] , setNext(),getNext() , @draw();
...
class GraphicEditor {
	...
	// 링크드 리스트 시작지점(고정),끝지점(변동),연결용(tmp),이동용(예비s)
	private int count; 	// 도형 갯수 저장
	...
	void inset(){
		...
		if(start == null) {	// 도형 처음 추가
			...
			else {System.out.println("잘못된 도형선택"); return;}
		}else {	//	이미 추가된거 뒤에 추가
			...
			else {System.out.println("잘못된 도형"); return;}
		}
		count++;
			...
			while (s != null) {
				if(sel == 1) {				// 첫번째 도형 삭제
					...
					if(s.getNext()==null) {		// 현재도형이 마지막 도형이면
						last = lastTmp;		//	마지막 도형 들고있는것을 이전거로 바꾸기
					}
					break;
				}
