...
class TicTacToe {
	...
	String [] theBoard = new String[NUM_BOARD];				//	보드 크기 10 고정
	Scanner sc;												//	입력받는게임
	String playerLetter;
	String computerLetter; 
	
	public TicTacToe() {									//	기본생성자 : 스캐너 생성함
	...
	void setPlayerLetter() {
		String letter = ""; // 용도?								//	X O 담아두기용 및 무한반복 트리거
		// 검사?
		while(!(letter.equals("X") || letter.equals("O"))) {		// X O 가 아니면 무한반복
			System.out.print("X를 하고 싶나요, O를 하고 싶나요? >>");
			letter = sc.nextLine().toUpperCase();					//	toUpperCase 모든 문자를 대문자로 변환
		}
		if(letter.equals("X")) {
			playerLetter = "X";
			computerLetter = "O";
	...
	String whoGoesFirst() {
		String turn = "player";										// 기본설정
		if((int)(Math.random()*2) == 0) {							//	0과1 랜덤
			turn = "computer";
		}
		return turn;
	}
	void drawBoard() {			//	 게임 보드 출력용 메소드
	...
	boolean isSpaceFree(int move) {			//	theBoard의 칸이 공백칸이면 ture다
	...
	int getPlayerMove() {
		String move = "0";
		while(!"123456789".contains(move) || !isSpaceFree(Integer.parseInt(move)) ) {	//첫회는 !false !true면 탈출
